...
function soall(){
	enginer="all";
	var site=getVByName();
	var url="<%=basePath%>so?engine=two&sotype=download";
	url=url+"&site="+site;
...
function sogoogle(){
	enginer="google";
	var site=getVByName();
	var url="<%=basePath%>so?engine=google&sotype=download";
	url=url+"&site="+site;
	var key2=$("input[name='keytext']").val();
	if(key2=="" || key2.length==0){
		return;
...
function sobaidu(){
	enginer="baidu";
	var site=getVByName();
	var url="<%=basePath%>so?engine=baidu&sotype=download";
	url=url+"&site="+site;
	var key2=$("input[name='keytext']").val();
	if(key2=="" || key2.length==0){
		return;
...
function onintchange(value){
	...
	<center>
		...
		<br/>
		<br/>
		<input type="radio" name="sun" value="baidu" checked="checked" onchange="onintchange(this.value)"/><img alt="百度文库" title="百度文库" src="img/download/baidu.jpg"/>&nbsp;百度文库
		<input type="radio" name="sun" value="csdn" onchange="onintchange(this.value)"/><img alt="CSDN" title="CSDN" src="img/download/csdn.jpg"/>&nbsp;CSDN
		<input type="radio" name="sun" value="51cto" onchange="onintchange(this.value)"/><img alt="51CTO" title="51CTO" src="img/download/51cto.jpg"/>&nbsp;51CTO
		<input type="radio" name="sun" value="chinaunix" onchange="onintchange(this.value)"/><img alt="ChinaUnix" title="ChinaUnix" src="img/download/chinaunix.jpg"/>&nbsp;ChinaUnix
		<input type="radio" name="sun" value="verycd" onchange="onintchange(this.value)"/><img alt="电驴" title="电驴" src="img/download/verycd.jpg"/>&nbsp;电驴
		<input type="radio" name="sun" value="xuexi" onchange="onintchange(this.value)"/><img alt="学习资料库" title="学习资料库" src="img/download/xuexi.jpg"/>&nbsp;学习资料库
		<input type="radio" name="sun" value="xidong" onchange="onintchange(this.value)"/><img alt="西东网" title="西东网" src="img/download/xidong.jpg"/>&nbsp;西东网
		<input type="radio" name="sun" value="eshuge" onchange="onintchange(this.value)"/><img alt="E书阁" title="E书阁" src="img/download/eshuge.jpg"/>&nbsp;E书阁
		<input type="radio" name="sun" value="daxues" onchange="onintchange(this.value)"/><img alt="中国大学生" title="中国大学生" src="img/download/daxues.jpg"/>&nbsp;中国大学生
		<input type="radio" name="sun" value="gaokao" onchange="onintchange(this.value)"/><img alt="高考资源网" title="高考资源网" src="img/download/gaokao.jpg"/>&nbsp;高考资源网
	...
	if(first=="true"){
		...
		if(key2.length>0){
			...
			url=url+"&sotype=download&site=baidu";
			$.post(url,{key:key2},function(data){
		    	//alert(data);
				var url =data.split("~");
				//alert(url[0]);
