...
function soall(){
	enginer="all";
	var site=getVByName();
	var url="<%=basePath%>so?engine=two&sotype=tieba";
	url=url+"&site="+site;
...
function sogoogle(){
	enginer="google";
	var site=getVByName();
	var url="<%=basePath%>so?engine=google&sotype=tieba";
	url=url+"&site="+site;
	var key2=$("input[name='keytext']").val();
	if(key2=="" || key2.length==0){
		return;
...
function sobaidu(){
	enginer="baidu";
	var site=getVByName();
	var url="<%=basePath%>so?engine=baidu&sotype=tieba";
	url=url+"&site="+site;
	var key2=$("input[name='keytext']").val();
	if(key2=="" || key2.length==0){
		return;
...
function onintchange(value){
	...
	<center>
		...
		<br/>
		<br/>
		<input type="radio" name="sun" value="baidu" checked="checked" onchange="onintchange(this.value)"/><img alt="百度贴吧" title="百度贴吧" src="img/tieba/baidu.jpg"/>&nbsp;百度贴吧
	...
	if(first=="true"){
		...
		if(key2.length>0){
			...
			url=url+"&sotype=tieba&site=baidu";
			$.post(url,{key:key2},function(data){
		    	//alert(data);
				var url =data.split("~");
				//alert(url[0]);
