...
function soall(){
	enginer="all";
	var site=getVByName();
	var url="<%=basePath%>so?engine=two&sotype=wenxian";
	url=url+"&site="+site;
...
function sogoogle(){
	enginer="google";
	var site=getVByName();
	var url="<%=basePath%>so?engine=google&sotype=wenxian";
	url=url+"&site="+site;
	var key2=$("input[name='keytext']").val();
	if(key2=="" || key2.length==0){
		return;
...
function sobaidu(){
	enginer="baidu";
	var site=getVByName();
	var url="<%=basePath%>so?engine=baidu&sotype=wenxian";
	url=url+"&site="+site;
	var key2=$("input[name='keytext']").val();
	if(key2=="" || key2.length==0){
		return;
...
function onintchange(value){
	...
	<center>
		...
		<br/>
		<br/>
		<input type="radio" name="sun" value="cnki" checked="checked" onchange="onintchange(this.value)"/><img alt="中国知网" title="中国知网" src="img/wenxian/cnki.jpg"/>&nbsp;中国知网
		<input type="radio" name="sun" value="wanfang" onchange="onintchange(this.value)"/><img alt="万方数据" title="万方数据" src="img/wenxian/wanfang.jpg"/>&nbsp;万方数据
		<input type="radio" name="sun" value="cqvip" onchange="onintchange(this.value)"/><img alt="维普网" title="维普网" src="img/wenxian/cqvip.jpg"/>&nbsp;维普网
		<input type="radio" name="sun" value="paperonline" onchange="onintchange(this.value)"/><img alt="中国科技论文在线" title="中国科技论文在线" src="img/wenxian/paperonline.jpg"/>&nbsp;中国科技论文在线
		<input type="radio" name="sun" value="lunwentianxia" onchange="onintchange(this.value)"/><img alt="论文天下" title="论文天下" src="img/wenxian/lunwentianxia.jpg"/>&nbsp;论文天下
		<input type="radio" name="sun" value="kexuenet" onchange="onintchange(this.value)"/><img alt="科学网" title="科学网" src="img/wenxian/kexuenet.jpg"/>&nbsp;科学网
		<input type="radio" name="sun" value="360lunwen" onchange="onintchange(this.value)"/><img alt="360论文网" title="360论文网" src="img/wenxian/360lunwen.jpg"/>&nbsp;360论文网
	...
	if(first=="true"){
		...
		if(key2.length>0){
			...
			url=url+"&sotype=wenxian&site=baidu";
			$.post(url,{key:key2},function(data){
		    	//alert(data);
				var url =data.split("~");
				//alert(url[0]);
